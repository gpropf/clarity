...
class CLNodeFactory {
   public:
    ...
    std::function<void()> stateFunction_ = [&]() {};
    ...
    Nc<V> *build(Nc<V> *existingNode = nullptr) {
        ...
        newNode->setStateFunction(stateFunction_);
    ...
    INLINE CLNodeFactory withStateFunction(std::function<void()> stateFunction) const & {
        CLNodeFactory cpy(*this);
        cpy.stateFunction_ = stateFunction;
        return cpy;
    }

    INLINE CLNodeFactory withStateFunction(std::function<void()> stateFunction) && {
        CLNodeFactory cpy(std::move(*this));
        cpy.stateFunction_ = stateFunction;
        return cpy;
    }
